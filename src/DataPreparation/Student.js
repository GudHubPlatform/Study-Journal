...
export default class StudentDataPreparation {
	...
	async getInterpretatedData(items) {
		...
		for (const item of items) {
			...
			const foundSubjectField = item.fields.find(field => field.field_id == subject_field_id);
			const subject = foundSubjectField ? foundSubjectField.field_value : null;
	...
	async getSubjectTitleMapRefIdAndInterpretation(items) {
		const { journal_app_id, subject_field_id } = this.scope.field_model.data_model;
		const map = new Map();
		const promises = [];

		for (const item of items) {
			const foundField = item.fields.find(field => field.field_id == subject_field_id);
			if (!foundField) continue;
			if (!map.has(foundField.field_value)) {
				const promise = gudhub.getInterpretationById(journal_app_id, item.item_id, subject_field_id, 'value')
